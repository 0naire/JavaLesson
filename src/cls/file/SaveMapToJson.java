...
public class SaveMapToJson {
    public static void main(String[] args) {
        ...
        String name = sc.nextLine();    // 키보드로부터 문자열을 입력 받아서 name이라는 변수에 저장한다.

        System.out.print("나이를 입력하세요 >>> ");
        int age = Integer.parseInt(sc.nextLine());     // 키보드로부터 정수를 입력 받아서 age라는 변수에 저장한다.

        System.out.print("키(실수)를 입력하세요 >>> ");
        float height = Float.parseFloat( sc.nextLine() );

        System.out.print("지역을 입력하세요 >>> ");
        String city = sc.nextLine();    // 키보드로부터 문자열을 입력 받아서 city라는 변수에 저장한다.

        // 맵 객체에 입력 받은 데이터를 저장한다
        myMap.put("name", name);
        myMap.put("age", age);
...
//        System.out.println( myMap );
        ...
        // gson 객체를이용하여 myMap 데이터를 JSON 형식 문자열로 변환
        ...
        try( FileWriter fd = new FileWriter(filename) )
        {
            ...
            fd.write( strJson );
            System.out.printf("%s에 데이터를 저장 하였습니다.\n", filename);
        } catch (IOException e) {
